...
public class TopicMasterServices {
	...
	public static Map<String, Object> findAllTopics(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		EntityExpr makeCondition = EntityCondition.makeCondition(makeCondition1, EntityComparisonOperator.OR,
				makeCondition2);
		try {
			topicGenericValueList = EntityQuery.use(delegator).from(CommonConstants.TOPIC_MASTER).where(makeCondition)
					.cache().queryList();
	...
	public static Map<String, Object> findTopicById(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			topicGV = EntityQuery
						.use(delegator)
						.from(CommonConstants.TOPIC_MASTER)
						.where(CommonConstants.TOPIC_ID, topicId)
						.cache()
						.queryOne();
			
		} catch (GenericEntityException e) {
			String errMsg = "Exception occured while fetching the record from TopicMaster entity: " + e.getMessage();
			Debug.logError(e, errMsg, module);
			return ServiceUtil.returnError(errMsg + module);
		}

		// If the Retrieved topics are empty
		if (UtilValidate.isEmpty(topicGV)) {
			// If the genericValueList is empty return error map
			String errMsg = "Retreived record list from TopicMasterEntity is null or empty";
			Debug.logWarning(errMsg, module);
			return ServiceUtil.returnError(errMsg + module);
		 ...
		 topicId = topicGV.getString(CommonConstants.TOPIC_ID);
		 String topicName = topicGV.getString(CommonConstants.TOPIC_NAME);
		 
		 serviceResultMap.put(CommonConstants.TOPIC_ID, topicId);
		 serviceResultMap.put(CommonConstants.TOPIC_NAME, topicName);
