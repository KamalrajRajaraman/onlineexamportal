...
public class ExamMasterServices {
	...
	public static Map<String, Object> findAllExams(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			//examsList = EntityQuery.use(delegator).from(CommonConstant.EXAM_MASTER).cache(true).queryList();
			...
			if (UtilValidate.isNotEmpty(examMasterGenericValueList)) {
				...
				for(GenericValue examGenericValue:examMasterGenericValueList) {
					...
					Map<String, Object> exam = UtilMisc.toMap(
	...
	public static Map<String, Object> findExamById(DispatchContext dctx, Map<String, ? extends Object> context) {
		...
		try {
			//examsList = EntityQuery.use(delegator).from(CommonConstant.EXAM_MASTER).cache(true).queryList();
			examGenericValue = EntityQuery.use(delegator).from(CommonConstant.EXAM_MASTER).where(CommonConstant.EXAM_ID,examIdPK).queryOne();
	...
	public static  Map<String, Object> getExamDetails(DispatchContext dctx, Map<String, ? extends Object> context){
		...
		try {
			examMasterGenericValueList = EntityQuery.use(delegator).from("ExamDetails").where(CommonConstant.EXAM_ID,examIdPK).queryList();
		} catch (GenericEntityException e) {
			e.printStackTrace();
		}
		...
		if (UtilValidate.isNotEmpty(examMasterGenericValueList)) {	
			...
			for(GenericValue examGenericValue:examMasterGenericValueList) {
				...
				Map<String, Object> exam = UtilMisc.toMap(
						...
						CommonConstant.NEGATIVE_MARK_VALUE, negativeMarkValue,
						//Topic
						CommonConstant.TOPIC_ID, topicId,
						CommonConstant.TOPIC_NAME,topicName,
						//ETMM
						CommonConstant.PERCENTAGE, percentage,
						CommonConstant.TOPIC_PASS_PERCENTAGE, topicPassPercentage,
						CommonConstant.QUESTION_PER_EXAM, questionPerExam,
						//QM
						CommonConstant.QUESTION_ID, questionId,
						CommonConstant.QUESTION_DETAIL, questionDetail,
						CommonConstant.OPTION_A, optionA,
						CommonConstant.OPTION_B, optionB,
						CommonConstant.OPTION_C, optionC,
						CommonConstant.OPTION_D, optionD,
						CommonConstant.OPTION_E, optionE,
						CommonConstant.ANSWER, answer,
						CommonConstant.NUM_ANSWERS, numAnswers,
						CommonConstant.QUESTION_TYPE, questionType,
						CommonConstant.DIFFICULTY_LEVEL, difficultyLevel,
