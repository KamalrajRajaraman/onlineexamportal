...
import {
  ...
  const [userAttemptTopicMasterList, setUserAttemptTopicMasterList] =
    useState();
  ...
  const fetchUserAttemptTopicMaster = async () => {
    const res = await fetch(
      PROTOCOL +
        ...
        WEB_APPLICATION +
        CONTROL_SERVLET +
        `findUserAttemptTopicMasterByPerforamceId?performanceId=${performanceId}`,
      GET
    );
  ...
  return (
    <>
      ...
      <UserAttemptDetailsTable
        userAttemptRecord={userAttemptRecord}
        topicResult={userAttemptTopicMasterList}
      />
    </>
  );
};
